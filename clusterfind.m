...
%            Frederik Gr??ll <Frederik.Gruell@kip.uni-heidelberg.de>%
%% 
...
THRESHOLD = (Threshold-1) * sqrt(meanbg);
    ...
    for i=1:size(ypos,1)
        ...
        if (x > ROIRADIUS && y > ROIRADIUS && x <= sizex - ROIRADIUS && y <= sizey - ROIRADIUS)
           ...
           if( max(fmid_roi(:))==firimg(y,x) )
               ...
               [maxQf,mxf,myf,dx,dy,sx2,sy2,Qf]= estimator( roi,meanbg );
               ...
               [maxQf,mx,my,dxf,dyf,sx2f,syf2,Qf]= estimator( roi,meanbg );
